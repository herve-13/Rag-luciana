...
log "Demarrage Ollama CPU pour embeddings retrieval..."
...
log "Demarrage API ChatFriends Retrieval..."
if curl -fs http://127.0.0.1:8002/healthz > /dev/null 2>&1; then
    log "API retrieval deja active sur :8002"
else
    if [ ! -d venv_wsl ]; then
    ...
    nohup env PYTHONPATH="$SCRIPT_DIR/src" \
        "$VENV_PY" -m uvicorn chatfriends_retrieval.api.main:app --host 0.0.0.0 --port 8002 \
        > "$LOG_DIR/retrieval_service.out" 2>&1 &
    for i in $(seq 1 45); do
        if curl -fs http://127.0.0.1:8002/healthz > /dev/null 2>&1; then
            log "OK: ChatFriends Retrieval (127.0.0.1:8002)"
            break
        fi
        sleep 1
    done
