#!/usr/bin/env python3
"""Export Luciana Qdrant collections to a readable Markdown file."""
...
from rag_luciana.clients.qdrant_client import get_qdrant_client


def _iter_luciana_collection_names(*, include_global: bool, include_private: bool) -> list[str]:
    client = get_qdrant_client()
    names = [c.name for c in client.get_collections().collections if str(c.name).startswith("rag_luciana_")]
    allowed_scopes = set()
    if include_global:
...
def export_markdown(*, output_path: Path, include_global: bool, include_private: bool) -> Path:
    collection_names = _iter_luciana_collection_names(
        include_global=include_global,
        include_private=include_private,
    )
    lines = [
        "# Luciana Qdrant Vectors Dump",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export Luciana Qdrant vectors to Markdown.")
    parser.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parent.parent / "docs" / "luciana_qdrant_vectors_dump.md"),
