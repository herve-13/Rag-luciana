...
from chatfriends_retrieval.api import ingest_router as ingest_module
...
def test_ingest_forwards_tenant_id_to_storage(monkeypatch):
    ...
    monkeypatch.setattr(ingest_module.settings, "rag_ingestion_enabled", True)
...
def test_ingest_returns_503_when_rag_ingestion_is_disabled(monkeypatch):
    app = FastAPI()
    app.include_router(ingest_router)
    ...
    app.dependency_overrides[_get_db] = fake_db
    app.dependency_overrides[verify_admin_key] = fake_admin
    monkeypatch.setattr(ingest_module.settings, "rag_ingestion_enabled", False)
    ...
    assert response.json()["detail"] == "rag_ingestion_disabled"
